...
@Component({
  ...
  standalone: true,
  imports: [MatButtonModule, MatDialogModule, ProductDetailsComponent, CommonModule],
...
export class ProductItemComponent {
  ...
  ngOnInit() {
    ...
    const productsInStore = this.productCartStoreService.activeCartProducts.value;
    const existingCartElement = productsInStore.filter(el => el.id === this.product.id)
    this.inStore = existingCartElement.length != 0 ? existingCartElement[0].amountInCart : 0;

    this.productCartStoreService.getCartProducts().subscribe(
      res => {
  ...
  addItemToCart = (product: Product)  => {
    ...
    const existingCartProductIndex = cartProducts.findIndex(el => el.id === newCartProduct.id);
    ...
    if (existingCartProductIndex >= 0) {
      ...
      this.productCartStoreService.updateCartProducts([...cartProducts, newCartProduct]);
    }
  } 
